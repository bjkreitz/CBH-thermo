...
for i, ax in enumerate(ax):
    ...
    axs[i].set_xlabel('$\mathrm{\Delta H\ (kJ\,mol^{-1})}$')    
    ...
    for no,spcs in enumerate(plots[i]):
        ...
        axs[i].barh(no+0.35,-results[i].iloc[no,2], height=0.2, color=colors[2], edgecolor='k', linewidth=3)
        
        if results[i].iloc[no,1]-results[i].iloc[no,0] <0:    
            axs[i].text(15, no, string_before+spcs+string_end, ha='left',va='center',bbox=props, fontsize='16')
        else:
            axs[i].text(-15, no, string_before+spcs+string_end, ha='right',va='center',bbox=props, fontsize='16')

#Create the legend for one of the plots
axs[2].barh(no-0.3+100,results_o.iloc[0,0] , height=0.3, color=colors[1], edgecolor='k', linewidth=3,label='$\mathrm{\Delta\Delta_f H}$')
axs[2].barh(no-0.3+100,results_o.iloc[0,0] , height=0.3, color=colors[2], edgecolor='k', linewidth=3,label='$\mathrm{\Delta H_{rxn}}$')
axs[2].plot((-30,-30),(-1,len(binding_o)),linestyle='dashed',color='k',label='$\mathrm{\pm 30\, kJ\,mol^{-1}}$')
handles, labels = axs[2].get_legend_handles_labels()

axs[2].legend(handles=[handles[1],handles[2], handles[0]],loc='center', ncol=3, bbox_to_anchor=(0.5,-0.2))
