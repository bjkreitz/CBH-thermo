...
class Molecule:
        ...
        def __init__(self):
                ...
                self.Eref={'CH4':-316.411233,'H2O':-600.7389158,'H2':-31.79434939,'NH3':-442.6510481} #DFT energies of the reference species (ZPE corrected) in eV
                self.Eslab=-366518.2051 #DFT energy of the slab in eV
...
def compute_thermo(molecule):
    ...
    print('CH4= ' + str(molecule.composition['C'])) 
    print('H2O= ' + str(molecule.composition['O']))
    print('H2=' + str(molecule.composition['H']/2-2*molecule.composition['C']-molecule.composition['O']) )
    return molecule.dHf


test = Molecule()
#parse_input_file('energies/COOH.dat',test)
#compute_thermo(test)

enthalpy=[]
groups=[]
for filename in glob.iglob('energies/*.dat'):
    bits_dat_info=filename.split('.')
    bits_dat=bits_dat_info[0].replace('energies/','')
    groups.append(bits_dat)
    
    parse_input_file(filename,test)
    hf=compute_thermo(test)
    enthalpy.append(hf)

import pandas as pd
d={'species':groups,'enthalpy':enthalpy}
deltaHf=pd.DataFrame(d)
deltaHf_sorted=deltaHf.sort_values('species')